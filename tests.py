...
class TestFuzzyMath:
    ...
    def test_multiplication_with_negative_result(self):
        assert 'negative number' in fuzzy_math(-5, '*', 1)

    def test_multiplication_with_small_result(self):
        assert 'small number' in fuzzy_math(5, '*', 1)

    def test_multiplication_with_medium_result(self):
        assert 'medium-sized' in fuzzy_math(5, '*', 3)

    def test_multiplication_with_large_result(self):
        assert 'really big' in fuzzy_math(5, '*', 5)

    def test_invalid_operator(self):
        for operator in ['-', '/']:
            with pytest.raises(Exception) as exc_info:
                fuzzy_math(5, operator, 5)
            assert "I don't know how to do math" in str(exc_info)
